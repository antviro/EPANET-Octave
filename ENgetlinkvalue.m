...
## -*- texinfo -*-
## @deftypefn {Function File} {@var{value},@var{errorcode} =} ENgetlinkvalue(@var{id},@var{param})
## get a value of a link@.@*
## Outputs:@*
## @var{value} is the value we wanteed to obtain@.@*
## @var{errorcode} can also be retrieved@.@*
## Inputs:@*
## @var{id} is the integer index of the link (can be a vector/matrix) or [] and then all link values are retrieved ENgetlinkvalue([],"EN_FLOW")@.@*
...
## EN_ENERGY 13 energy expended in kW (¿power?)@*
## @seealso{ENgetlinkid, ENgetlinkvalue, ENsetlinkvalue}
## @end deftypefn

function [value, errorcode] = ENgetlinkvalue(id,param)
  if ischar(param),
    param=toupper(param);
    switch param,
      case "EN_DIAMETER"
  ...
  if  length(id)==1,
    [value,errorcode] = ocENgetlinkvalue(id, paramcode);
  elseif length(id)>1,
    [value,errorcode] = arrayfun ("ocENgetlinkvalue",id,paramcode);
