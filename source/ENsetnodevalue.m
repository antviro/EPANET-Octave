...
## -*- texinfo -*-
## @deftypefn {Function File} {[@var{errorcode}] =} ENsetnodevalue(@var{id},@var{param},@var{value})
## Sets the value in a node@.@*
## Outputs:@*
## @var{errorcode} can also be retrieved@.@*
## Inputs:@*
## @var{id} is the integer index of the node (can be a vector/matrix). If no node is specified all nodes value are retrieved e.g. ENsetnodevalue ([],10)@.@*
## @var{param} is the parameter to be measured it can be a string or an integer:@*
## @var{value} is the values we want to set it should have the same size of id (if id is empty then it should be a vector with one value for each node.@.@*
## "EN_ELEVATION" 0                               @*
## "EN_BASEDEMAND" 1                               @*
## "EN_PATTERN" 2                                  @*
...
## "EN_SETPOINT" 2@*
## "EN_FLOWPACED" 3@*
## @seealso{ENsetnodeid, ENsetnodevalue, ENsetnodevalue}
## @end deftypefn

function [errorcode] = ENsetnodevalue(id,param,value)
  if ischar(param),
    param=toupper(param);
    switch param,
      case "EN_ELEVATION"
  ...
  if  length(id)==1,
    errorcode = ocENsetnodevalue(id, paramcode,value);
  elseif length(id)>1,
    errorcode = arrayfun ("ocENsetnodevalue",id,paramcode,value); 
