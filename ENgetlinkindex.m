...
## -*- texinfo -*-
## @deftypefn {Function File} {@var{index},@var{errorcode} =} ENgetlinkindex(@var{linkid})
## Runs a step of hydraulic simulation.
## @var{linkid} is a string with the link identifier, it should be a string variable containing the string and not the string itself. No idea of why it cause problem the later...
...
function [index,errorcode] = ENgetlinkindex(linkid)
  if  ~iscell(linkid),
    [index,errorcode] = ocENgetlinkindex(linkid);
  else
    [index,errorcode] = cellfun ("ocENgetlinkindex",linkid);
  endif
endfunction
